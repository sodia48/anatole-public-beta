...
class ConnectionAdapter:
    ...
    def execute(self, query: str, params: Any = ()):
        return self.raw.execute(_convert_placeholders(query, self.backend), params)

    def executemany(self, query: str, params: Any):
        converted_query = _convert_placeholders(query, self.backend)
...
ON notifications(profile, created_at);

CREATE INDEX IF NOT EXISTS idx_notifications_profile_read
ON notifications(profile, is_read);

CREATE INDEX IF NOT EXISTS idx_alerts_profile_active
ON alerts(profile, active);

CREATE INDEX IF NOT EXISTS idx_alert_events_profile_created
ON alert_events(profile, created_at);
...
def ensure_profile(profile: str) -> str:
    ...
    if clean in _INITIALIZED_PROFILES:
        return clean
    ...
    with _PROFILE_LOCK:
        ...
        _INITIALIZED_PROFILES.add(clean)
    return clean
...
def delete_profile_data(profile: str) -> None:
    ...
    _INITIALIZED_PROFILES.discard(profile)
...
def count_unread_notifications(profile: str) -> int:
    with connection() as conn:
        row = conn.execute(
            """
            SELECT COUNT(*) AS total
            FROM notifications
            WHERE profile=? AND is_read=0
            """,
            (profile,),
        ).fetchone()
...
def set_preferences(profile: str, values: dict[str, str]) -> None:
    if not values:
        return
    records = [
...
def set_preference(profile: str, key: str, value: str) -> None:
    set_preferences(profile, {key: value})


def get_preferences(profile: str) -> dict[str, str]:
    with connection() as conn:
        rows = conn.execute(
            """
    ...
    return {
        str(row["pref_key"]): str(row["pref_value"])
        for row in rows
    }


def get_preference(profile: str, key: str, default: str = "") -> str:
    return get_preferences(profile).get(key, default)
