...
from core.config import EXPECTED_CONSTITUENTS, TORONTO_TZ
...
def live_cockpit() -> None:
    ...
    if market.empty:
        st.error("Les données de marché ne sont pas disponibles. Réessaie dans quelques instants.")
        return
    ...
    if "Market Pulse" in visible_modules or not visible_modules:
        ...
        metrics[0].metric("TSX 60 indicatif", f"{weighted_change:+.2f}%" if not np.isnan(weighted_change) else "N/D")
...
if diagnostics.get("actual") != EXPECTED_CONSTITUENTS:
    missing_count = max(
        0,
        EXPECTED_CONSTITUENTS - int(diagnostics.get("actual", 0) or 0),
    )
    st.warning(
        f"La composition récupérée contient {diagnostics.get('actual', 0)} titres "
        f"sur {EXPECTED_CONSTITUENTS}. Anatole continue avec les titres disponibles."
    )
